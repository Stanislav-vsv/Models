...
lower_oil = pd.DataFrame(oil.iloc[-1,:].copy()).T
lower_oil['VALUE_DAY'] += pd.Timedelta(180,unit='d')
upper_oil = pd.DataFrame(oil.iloc[0,:].copy()).T
upper_oil['VALUE_DAY'] -= pd.Timedelta(180,unit='d')
oil = pd.concat([upper_oil,oil,lower_oil],axis=0)
oil['TRADE_OPEN'] = oil['TRADE_OPEN'].astype(float)
oil.index = pd.to_datetime(oil['VALUE_DAY'])
oil = oil.resample("1d").interpolate("time")[['TRADE_OPEN']]
oil.columns=['value']
oil['DATE'] = oil.index
oil.fillna(method='pad')
oil.name = 'oil'
...
#++++++++++++++++++++++++++++
# miacr = pd.read_csv('MIACR.txt',sep=';')
...
lower_miacr['VALUE_DAY'] += pd.Timedelta(180,unit='d')
upper_miacr = pd.DataFrame(miacr.iloc[0,:].copy()).T
upper_miacr['VALUE_DAY'] -= pd.Timedelta(180,unit='d')
...
miacr = miacr.resample("1d").interpolate("nearest")[['LAST_QUOTE_CLOSE']]
miacr.columns=['value']
...
#++++++++++++++++++++++++++++
# gdp = pd.read_csv('GDP.txt',sep=';')
# gdp['VALUE_DAY'] = pd.to_datetime(gdp['VALUE_DAY'],format='%d.%m.%Y')
#++++++++++++++++++++++++++++
lower_gdp = pd.DataFrame(gdp.iloc[-1,:].copy()).T
lower_gdp['VALUE_DAY'] -= pd.Timedelta(195,unit='d')
upper_gdp = pd.DataFrame(gdp.iloc[0,:].copy()).T
upper_gdp['VALUE_DAY'] += pd.Timedelta(1080,unit='d')
gdp = pd.concat([upper_gdp,gdp,lower_gdp],axis=0)
gdp.index = pd.to_datetime(gdp['VALUE_DAY'])
gdp['value'] = gdp['GDP_USD_BN_2016'].astype(int)
gdp.drop(['VALUE_DAY','GDP_USD_BN_2016','GDP_USD_BN_2011'],axis=1,inplace=True)
gdp = gdp.resample("1d").interpolate("nearest")
gdp['DATE'] = gdp.index
gdp.name = 'gdp'
...
#++++++++++++++++++++++++++++
# inf_unemp = pd.read_csv('INFLAT_UNEMPLOY.txt',sep=';')
# inf_unemp['VALUE_DAY'] = pd.to_datetime(inf_unemp['VALUE_DAY'],format='%d.%m.%Y')
#++++++++++++++++++++++++++++
...
upper_inf_unemp['VALUE_DAY'] += pd.Timedelta(720,unit='d')
...
inf_unemp = inf_unemp.resample("1d").interpolate("nearest")
...
################### БЕЗРАБОТИЦА ###################
unemp = inf_unemp[['DATE','UNEMPLOY']].copy()
...
#++++++++++++++++++++++++++++
# exch = pd.read_csv('EXCHANGERATE_STAT.txt',sep=';')
# exch['VALUE_DAY'] = pd.to_datetime(exch['VALUE_DAY'],format='%d.%m.%Y')
#++++++++++++++++++++++++++++
...
euro, dollar = exch[exch['CURRENCY_TO_CCODE'] == 'EUR'][['VALUE_DAY','RATE']], exch[exch['CURRENCY_TO_CCODE'] == 'USD'][['VALUE_DAY','RATE']]
lower_euro = pd.DataFrame(euro.iloc[0,:].copy()).T
lower_euro['VALUE_DAY'] -= pd.Timedelta(365,unit='d')
upper_euro = pd.DataFrame(euro.iloc[-1,:].copy()).T
upper_euro['VALUE_DAY'] += pd.Timedelta(365,unit='d')
euro = pd.concat([lower_euro,euro,upper_euro],axis=0)
euro.drop_duplicates(inplace=True)
euro.index = pd.to_datetime(euro['VALUE_DAY'])
euro['RATE'] = euro['RATE'].astype(float)
euro = euro.resample("1d").interpolate("time")[['RATE']]
euro.columns=['value']
euro['DATE'] = euro.index
euro.name = 'euro'

lower_dollar = pd.DataFrame(dollar.iloc[0,:].copy()).T
lower_dollar['VALUE_DAY'] -= pd.Timedelta(365,unit='d')
upper_dollar = pd.DataFrame(dollar.iloc[-1,:].copy()).T
upper_dollar['VALUE_DAY'] += pd.Timedelta(365,unit='d')
dollar = pd.concat([lower_dollar,dollar,upper_dollar],axis=0)
dollar.drop_duplicates(inplace=True)
dollar.index = pd.to_datetime(dollar['VALUE_DAY'])
dollar['RATE'] = dollar['RATE'].astype(float)
dollar = dollar.resample("1d").interpolate("time")[['RATE']]
...
# data = pd.read_csv('DEALGUARANTEE.txt',sep=';',encoding='CP1251')
...
data['DEAL_START_AMOUNT'] = data['DEAL_START_AMOUNT'].astype(float)
data['USED_AMOUNT'] = data['USED_AMOUNT'].astype(float)

data.index = np.arange(data.shape[0])
def sum_to_roubles(price,curr_type,time):
    if curr_type == 'USD':
        return (price * float(dollar[dollar['DATE'] == time]['value'][0]))
    elif curr_type == 'EUR':
        return (price * float(euro[euro['DATE'] == time]['value'][0]))
    else:
        return price
...
def fit_predict(model_type, X_train, y_train, X_test, estimator_type):
    if model_type == 'bau':
        threshold = 0.1
...
bau_result = fit_predict('bau', data_train.loc[:,'DEAL_CURRENCY':], target_train, data_test.loc[:,'DEAL_CURRENCY':], 'catboost')

stress_result = fit_predict('stress', data_stress_train.loc[:,'DEAL_CURRENCY':], target_stress_train, data_stress_test.loc[:,'DEAL_CURRENCY':], 'catboost')
stress_result = max(stress_result,bau_result)
